...
ls -d results/*/ | \
    sed -e 's/\([0-9]*\)//g' \
     -e 's/._\///g' -e 's/results\///g' |\
     uniq > list

for i in  $(cat list) ; do 
    mkdir results/"$i".glob
    mv results/"$i".h*_* results/"$i".glob ; 
    for k in $(find results/"$i".glob -name ABCstat.txt) ; do 
        cat "$k" |grep -v dataset >> results/"$i".ABC.stat.txt ; 
    done   
   for k in $(find results/$i.glob -name priorfile) ; do 
       cat "$k" |grep -v N1 >> results/"$i".priorfile.txt ; 
   done ; 
done
