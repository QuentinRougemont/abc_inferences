...
cp ../bpfile . 
cp ../spinput.txt .
cp ../results/sc1.prior.from.post .
sed -i '/^$/d' sc1.prior.from.post

#global variable
outfile=gfit.outfile
nlines=$(wc -l sc1.prior.from.post |sed -e 's/sc1.prior.from.post//g') 
nreps=1
...
while read line; do 
        echo "$line" > tmp
        col1=$(awk '{print $6}' tmp) #$10=M1
        col2=$(awk '{print $7}' tmp) #$11=M2
        ...
        ../bin/msnsam $ntot $(( $nreps * $nlocus )) -s 1 -I 2 $nsp1 $nsp2 0 -m 1 2 $col1 -m 2 1 $col2 -n 1 $col3 -n 2 $col4 -eM $col5 0 -ej $col6 2 1 -eN $col7 $col8 > tmp.ms
