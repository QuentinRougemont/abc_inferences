...
nlinesFul <- as.numeric(strsplit(system("wc -l results/eq.homom.homon.ABC.stat.txt ", intern=T), " ")[[1]][1])
M1 <- matrix(scan("results/eq.homom.homon.ABC.stat.txt"), byrow=T, nrow=nlinesFul)
...
#load priorfile
priorfile=matrix(scan("results/eq.homom.homon.priorfile.txt"), 
    byrow=T, 
    nrow=nlinesFul) 
...
write.table(newsamp,"eq.prior.from.post.homo", quote=F, row.names=F, col.names=F)
