...
colnames(priorfile) = c("N1","N2","Na","Tsplit","Tsc","Shape1Ne","Shape2Ne","propNtrN1","proprNtrN2","M1","M2")
...
abc.postSC <- abc(target  = obs, param = priorfile, sumstat = M_SC3b, tol = 1000/1e6, transf=c(rep("logit",11) ),
         ...
         range(priorfile[, 6]),range(priorfile[, 7]),range(priorfile[, 8]),range(priorfile[, 9]),range(priorfile[, 10]), range(priorfile[, 11])),
...
n1.n2[,1] <- rbeta(nlocus,newsamp[,6], newsamp[,7]) * newsamp[,1]
n1.n2[,2] <- rbeta(nlocus,newsamp[,6], newsamp[,7]) * newsamp[,2]
n1.tmp  <- sample(n1, simpost2)
...
newsamp[,1] <- n1
newsamp[,2] <- n2
write.table(newsamp,"prior.from.post.heteron.homom", quote=F, row.names=F, col.names=F)
