...
nlinesFul=as.numeric(strsplit(system("wc -l am.homom.homon.ABC.stat.txt ", intern=T), " ")[[1]][1])
M_SC1=matrix(scan("am.homom.homon.ABC.stat.txt"), byrow=T, nrow=nlinesFul) 
...
#Hetero Hetero
priorfile=matrix(scan("am.homom.homon.priorfile.txt"), byrow=T, nrow=nlinesFul) 
...
sink("postadjval.am.homo.homo.tol0.1.txt")
print(abc.postSC$adj.values)
sink()

#Compute mean, mode, 95%HPD
sink("summary.post.am.homo.homo.tol0.1.txt")
print(summary(abc.postSC))
sink()


pdf(file="am.homo.homo.tol0.1.pdf",width=12,height=8)
par(mfrow=c(4,4))
...
plot( prior.T,
      ...
      xlab="T am",
      lwd=2, 
      main="Time of ancient migration")
lines(posterior.T, col="blue",lwd=2)
...
#############################################################Tol 0.02############################################"
abc.postSC <- abc(target  = obs, param = priorfile, sumstat = M_SC1b, tol = 250/1e6, transf=c(rep("logit",7) ), 
	logit.bounds = rbind(range(priorfile[, 1]),range(priorfile[, 2]), range(priorfile[, 3]), range(priorfile[, 4]), range(priorfile[, 5]),range(priorfile[, 6]),range(priorfile[, 7])),
	hcorr=T, method  = "neuralnet", numnet=50, sizenet=15)

sink("postadjval.am.homo.homo.tol0.025.txt")
print(abc.postSC$adj.values)
sink()

#Compute mean, mode, 95%HPD
sink("summary.post.am.homo.homo.tol0.025.txt")
print(summary(abc.postSC))
sink()

pdf(file="am.homo.homo.tol0.025.pdf",width=12,height=8)
par(mfrow=c(4,4))
prior.tetha1     <- density( (priorfile[,1] ) )
posterior.tetha1 <- density( abc.postSC$adj.values[,1] , weights=abc.postSC$weights/sum(abc.postSC$weights) )
theta1 <- expression(theta[1])
...
##################################################
######################################################################################################################
M_SC4=matrix(scan("am.heterom.heteron.ABC.stat.txt"), byrow=T, nrow=nlinesFul) 
...
#Hetero Hetero
priorfile=matrix(scan("am.heterom.heteron.priorfile.txt"), byrow=T, nrow=nlinesFul) 
colnames(priorfile)=c("N1","N2","Na",      
						"Tsplit",
						"Tsc",  
						"shape1Ne",        
...
sink("postadjvalue.am.het.het.tol0.1.txt")
print(abc.postSC$adj.values)
sink()


#Compute mean, mode, 95%HPD
sink("summary.post.am.het.het.tol0.1.txt")
print(summary(abc.postSC))
sink()

pdf(file="am.het.het.tol0.1.pdf",width=12,height=8)
par(mfrow=c(4,4))
prior.tetha1     <- density( (priorfile[,1] ) )
posterior.tetha1 <- density( abc.postSC$adj.values[,1] , weights=abc.postSC$weights/sum(abc.postSC$weights) )
theta1 <- expression(theta[1])
...
prior.T    <- density( priorfile[,17] )
posterior.T <- density( abc.postSC$adj.values[,17] , weights=abc.postSC$weights/sum(abc.postSC$weights) )
print_tau <- expression(tau)
plot( prior.T,
      xlim=c(0,6000),
      ylim=c(0,0.2), 
...
#
########################################################################################################################################
M_SC3=matrix(scan("am.homom.heteron.ABC.stat.txt"), byrow=T, nrow=nlinesFul) 
...
#Hetero Hetero
priorfile=matrix(scan("am.homom.heteron.priorfile.txt"), byrow=T, nrow=nlinesFul) 
colnames(priorfile)=c("N1","N2","Na",      
						"Tsplit",
						"Tsc",  
						"shape1Ne",        
...
sink("postadj.val.am.homom.hetn.tol0.1.txt")
print(abc.postSC$adj.values)
sink()

#Compute mean, mode, 95%HPD
sink("summary.post.am.homom.hetn.tol0.1.txt")
print(summary(abc.postSC))
sink()


pdf(file="am.homom.hetn.tol0.1.pdf",width=12,height=8)
par(mfrow=c(4,4))
prior.tetha1     <- density( (priorfile[,1] ) )
posterior.tetha1 <- density( abc.postSC$adj.values[,1] , weights=abc.postSC$weights/sum(abc.postSC$weights) )
theta1 <- expression(theta[1])
...
###########################################" Tol 0.025 #####################################################"""###
abc.postSC <- abc(target  = obs, param = priorfile, sumstat = M_SC3b, tol = 250/1e6, transf=c(rep("logit",11) ), 
	logit.bounds = rbind(range(priorfile[, 1]),range(priorfile[, 2]), range(priorfile[, 3]), range(priorfile[, 4]), range(priorfile[, 5]),range(priorfile[, 6]),range(priorfile[, 7]),range(priorfile[, 8]),range(priorfile[, 9]),
	range(priorfile[, 10]), range(priorfile[, 11])),
	hcorr=T, method  = "neuralnet", numnet=50, sizenet=15)

sink("postadj.val.am.homom.hetn.tol0.025.txt")
print(abc.postSC$adj.values)
sink()

#Compute mean, mode, 95%HPD
sink("summary.post.am.homom.hetn.tol0.25.txt")
print(summary(abc.postSC))
sink()

pdf(file="am.homom.hetn.tol0.025.pdf",width=12,height=8)
par(mfrow=c(4,4))
prior.tetha1     <- density( (priorfile[,1] ) )
posterior.tetha1 <- density( abc.postSC$adj.values[,1] , weights=abc.postSC$weights/sum(abc.postSC$weights) )
theta1 <- expression(theta[1])
...
#
########################################################################################################################################
M_SC1=matrix(scan("am.heterom.homon.ABC.stat.txt"), byrow=T, nrow=nlinesFul) 
...
priorfile=matrix(scan("am.heterom.homon.priorfile.txt"), byrow=T, nrow=nlinesFul) 
colnames(priorfile)=c("N1","N2","Na",      
						"Tsplit",
						"Tsc",  
						"M1",
...
sink("postadj.val.am.hetero.homon.tol0.1.txt")
print(abc.postSC$adj.values)
sink()

#Compute mean, mode, 95%HPD
sink("summary.post.am.heterom.homon.tol0.1.txt")
print(summary(abc.postSC))
sink()

##################################################"
pdf(file="am.heterom.homon.tol0.1.pdf",width=12,height=8)
par(mfrow=c(4,4))
prior.tetha1     <- density( (priorfile[,1] ) )
posterior.tetha1 <- density( abc.postSC$adj.values[,1] , weights=abc.postSC$weights/sum(abc.postSC$weights) )
theta1 <- expression(theta[1])
