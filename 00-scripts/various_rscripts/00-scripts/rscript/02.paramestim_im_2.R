...
colon_count = paste(" awk -F ' ' '{print NF }' ", "00.data/im.heterom.homon.ABC.stat.txt" , " |head -1 " )
...
model <- M_IM1[c(1:nlinesFul),-c(1:3,12,13,18:25)]

colon_count_pri = paste(" awk -F ' ' '{print NF }' ", "00.data/im.heterom.homon.priorfile.txt" , " |head -1 " )
ncol_prior = as.numeric(strsplit(system ( colon_count_pri , intern=T), " ")[[1]] [1])

priorfile=matrix(scan("00.data/im.heterom.homon.priorfile.txt"), byrow=T, ncol=ncol_prior)
...
write.table(summary(abc.posterior), "im.2.parameter.estim.txt", quote=F,row.names=F,col.names=F)
write.table(abc.posterior$weights,  "im.2.weight.homo.heteroM",quote=F,row.names=F,col.names=F)
write.table(abc.posterior$adj.values,"im.2.adjval.homo.heteroM",quote=F,row.names=F)
pop <- strsplit(getwd(),"03.abc/")[[1]][2]

#One possible graph:
pdf(file=paste(pop, "im.2.pior.posterior.1000post.pdf", sep="."),width=12,height=8)
par(mfrow=c(3,3))
hist(priorfile[,1],
...
hist(priorfile[,11],
        breaks=seq(0,max(priorfile[,11]+0.1),1),
        col="lightgrey",
        freq=F,
        ylim=c(0,0.25),
        ...
        ylab="probab density")
#hist(abc.posterior$unadj.values[,6],   breaks=seq(0,max(priorfile[,7]+0.1),0.5), col=rgb(1,0,0,0.5), freq=F, add=T, )
wtd.hist(abc.posterior$adj.values[,11], breaks=seq(0,max(priorfile[,11]+0.1),0.5), col=rgb(0.8,0,0.3,0.2), freq=F, add=T, weight=abc.posterior$weights/sum(abc.posterior$weights))
box()
