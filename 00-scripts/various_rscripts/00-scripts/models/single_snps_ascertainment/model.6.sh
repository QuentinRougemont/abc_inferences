...
../../bin/priorgen5.py \
    ...
    M2=0 M2=40 shape1=0 shape1=20 shape2=0 shape2=200 model=AM nreps="$NREPS" \
    Nvariation=homo Mvariation=homo symMig=asym parameters=priorfile | \
    ../../bin/msnsam tbs $(( $NREPS * $NLOCUS )) -s 1 -I 2 \
        tbs tbs 0 -m 1 2 tbs -m 2 1 tbs -n 1 tbs -n 2 tbs -ema tbs 2 0 tbs tbs 0 -ej tbs 2 1 -eN tbs tbs | ../../bin/ascertainement_bias_4.py   $NLOC  $NLOCUS  > myfifo  &

../../bin/mscalc < myfifo #>/dev/null
rm  bpfile_10  error.txt  myfifo seedms  spinput.txt  spoutput.txt
