...
colon_count = paste(" awk -F ' ' '{print NF }' ", "00.data/sc.heterom.homon.ABC.stat.txt" , " |head -1 " )
...
colon_count_pri = paste(" awk -F ' ' '{print NF }' ", "00.data/sc.heterom.homon.priorfile.txt" , " |head -1 " )
ncol_prior = as.numeric(strsplit(system ( colon_count_pri , intern=T), " ")[[1]] [1])

priorfile=matrix(scan("00.data/sc.heterom.homon.priorfile.txt"), byrow=T, ncol=ncol_prior)
...
abc.posterior <- abc(target  = obs, param = priorfile, sumstat = M_SC2b, 
              tol = 1000/1e6, transf=c(rep("logit",ncol(priorfile) ) ),
              logit.bounds = logit.bound,
              hcorr=T, method  = "neuralnet", numnet=50, sizenet=15
               )

write.table(summary(abc.posterior),"sc.2parameter.estim.txt", quote=F,row.names=F,col.names=F)
write.table(abc.posterior$weights,"sc.2.weight.homo.homo",quote=F,row.names=F,col.names=F)
write.table(abc.posterior$adj.values,"sc.2.adjval.homo.homo",quote=F,row.names=F)
pop <- strsplit(getwd(),"03.abc/")[[1]][2]
#One possible graph:
pdf(file=paste(pop,"sc.2.pior.posterior.1000post.pdf",sep="."),width=12,height=8)
par(mfrow=c(3,3))
hist(priorfile[,1],
...
hist(priorfile[,4],
	...
	freq=F,
	ylim=c(0,0.3),
	main=c(expression(tau), " = Tsplit/4Nref"),cex.main=1.5,
	xlab= expression(tau),
	ylab="probability density")
...
hist(priorfile[,5],
	...
	freq=F,
	ylim=c(0,0.3),
	main="Tsc = TauSC/4Nref",cex.main=1.5,
	xlab= "Tsc",
	ylab="probab density")
...
hist(priorfile[,6],
	...
	xlab= "M1<-2",
	ylab="probab density")
#hist(abc.posterior$unadj.values[,6],   breaks=seq(0,max(priorfile[,5]+0.1),0.5), col=rgb(1,0,0,0.5), freq=F, add=T, )
...
hist(priorfile[,12],
        breaks=seq(0,max(priorfile[,12]+0.1),1),
        col="lightgrey",
        freq=F,
        ylim=c(0,0.25),
        main="Prop m neutral Sp1 ",cex.main=1.5,
        xlab= "m neutral" ,
        ylab="probab density")
#hist(abc.posterior$unadj.values[,6],   breaks=seq(0,max(priorfile[,7]+0.1),0.5), col=rgb(1,0,0,0.5), freq=F, add=T, )
wtd.hist(abc.posterior$adj.values[,12], breaks=seq(0,max(priorfile[,12]+0.1),0.5), col=rgb(0.8,0,0.3,0.2), freq=F, add=T, weight=abc.posterior$weights/sum(abc.posterior$weights))
box()

hist(priorfile[,13],
        breaks=seq(0,max(priorfile[,13]+0.1),1),
        col="lightgrey",
        freq=F,
        ylim=c(0,0.25),
        main="Prop m neutral Sp2 ",cex.main=1.5,
        xlab= "m neutral" ,
        ylab="probab density")
#hist(abc.posterior$unadj.values[,6],   breaks=seq(0,max(priorfile[,7]+0.1),0.5), col=rgb(1,0,0,0.5), freq=F, add=T, )
wtd.hist(abc.posterior$adj.values[,13], breaks=seq(0,max(priorfile[,13]+0.1),0.5), col=rgb(0.8,0,0.3,0.2), freq=F, add=T, weight=abc.posterior$weights/sum(abc.posterior$weights))
box()
