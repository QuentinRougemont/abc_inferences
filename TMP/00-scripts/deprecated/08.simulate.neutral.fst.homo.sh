...
Rscript ../00-scripts/rscript/05.generate.prior.from.post.best.homo.R
cd ..
...
ntot=$(( $nsp1 + $nsp2 ))
folder=04.sim.fst.he.homo

mkdir "$folder"
cd "$folder"    

cp ../results/prior.from.post.homo .
mv prior.from.post.homo prior.from.post 
